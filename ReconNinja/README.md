...
[![Version](https://img.shields.io/badge/version-5.1.0-6366f1?style=flat-square)](https://github.com/ExploitCraft/ReconNinja/releases)
...
Phase 12  v5 Integrations    WHOIS · Wayback · SSL · VirusTotal · Shodan · Censys · Hunter.io
Phase 13  Diff Analysis      Compare scans — delta report of changes (new in v5.1.0)
...
**3 new focus areas — 14 new features:**

### 🔍 Scan Intelligence
| Feature | Flag |
|---|---|
| Smart port deduplication | automatic — merges RustScan + AsyncTCP + Masscan + Nmap |
| Service confidence scoring | shown in CSV/JSON reports as `confidence` field |
| Custom Nmap scripts | `--nmap-scripts vuln` or `--nmap-scripts http-title,ssl-cert` |
| Confidence filtering | `--confidence-min 80` |

### 📊 Output & Reports
| Feature | Flag |
|---|---|
| Scan diff report | `--diff /path/to/old-state.json` |
| CSV export | `--output-format csv` |
| SARIF export (GitHub Code Scanning) | `--output-format sarif` |
| Custom Jinja2 HTML templates | `--report-template /path/to/template.html` |
| Severity summary banner | automatic — printed after every scan |

### 🔌 New Integrations
| Module | Flag | API Key |
|---|---|---|
| Censys host intelligence | `--censys --censys-key id:secret` | Free tier |
| Hunter.io email enumeration | `--hunter --hunter-key KEY` | Free tier |

**Plugin system upgrade:**
```python
# Plugins can now return VulnFinding lists — auto-appended to results
def run(target, out_folder, result, config):
    return [VulnFinding(tool="myplugin", severity="high", ...)]
```
