...
from core.diff import run_diff, render_diff_markdown
from output.reports import (
    generate_json_report, generate_html_report, generate_markdown_report,
    generate_csv_report, generate_sarif_report, generate_html_from_template,
...
def orchestrate(cfg: ScanConfig,
    ...
    # Censys  (v5.1.0)
    if cfg.run_censys and cfg.censys_key and result.hosts and "censys" not in result.phases_completed:
        console.print(Panel.fit("[phase] PHASE 12f — Censys Intelligence [/]"))
        # censys_key expected as "api_id:api_secret"
        parts = cfg.censys_key.split(":", 1)
        api_id, api_secret = (parts[0], parts[1]) if len(parts) == 2 else ("", "")
        ips = [h.ip for h in result.hosts if h.ip][:20]
        censys_data = censys_bulk_lookup(ips, api_id=api_id, api_secret=api_secret)
        result.censys_results.extend(censys_data)
        result.phases_completed.append("censys")
        save_state(result, cfg, out_folder)

    # Hunter.io  (v5.1.0)
    if cfg.run_hunter and cfg.hunter_key and "hunter" not in result.phases_completed:
        console.print(Panel.fit("[phase] PHASE 12g — Hunter.io Email Intelligence [/]"))
        hunter_data = hunter_domain_search(cfg.target, cfg.hunter_key)
        if hunter_data:
            result.hunter_results.append(hunter_data)
        result.phases_completed.append("hunter")
        save_state(result, cfg, out_folder)

    # ── Phase 13: Diff Analysis (v5.1.0) ─────────────────────────────────
    if cfg.diff_file and "diff" not in result.phases_completed:
        console.print(Panel.fit("[phase] PHASE 13 — Scan Diff Analysis [/]"))
        from dataclasses import asdict
        new_state_dict = asdict(result)
        new_state_dict["target"] = result.target
        new_state_dict["start_time"] = result.start_time
        diff_summary = run_diff(cfg.diff_file, new_state_dict)
        result.diff_summary = diff_summary
        safe_print(f"[info]Diff: {diff_summary.get('summary', 'done')}[/]")
        # Write standalone diff.md
        diff_md = render_diff_markdown(diff_summary)
        (out_folder / "diff.md").write_text(diff_md, encoding="utf-8")
        safe_print(f"[success]✔ Diff report: {out_folder / 'diff.md'}[/]")
        result.phases_completed.append("diff")
        save_state(result, cfg, out_folder)
    ...
    csv_path   = out_folder / "report.csv"
    ...
    if fmt in ("all", "html"):
        if cfg.report_template and cfg.report_template.exists():
            generate_html_from_template(result, cfg.report_template, html_path)
        else:
    ...
    if fmt in ("all", "csv"):
        ...
        console.print(f"[info]  CSV:   {csv_path}[/]")
    if fmt in ("all", "sarif"):
        generate_sarif_report(result, sarif_path)
        console.print(f"[info]  SARIF: {sarif_path}[/]")


    # ── Severity summary banner (v5.1.0) ─────────────────────────────────
    from output.reports import build_severity_summary
    sev_line = build_severity_summary(result)
    console.print(f"\n[bold]Severity Summary:[/] {sev_line}\n")
    ...
    console.print(Panel.fit(
        f"[success]✔ ReconNinja v5.1.0 Complete[/]\n"
