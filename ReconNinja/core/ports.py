...
def async_port_scan(
    ...
    for port in open_ports:
        ...
        port_infos.append(PortInfo(
            ...
            confidence = 70 if not banner else 85,
            source     = "async",
        ))
...
_NMAP_TOP_PORTS = [
    ...
    407,9998,2717,4899,1745,8883,1337,1338,10243,1024,58080,
...
# ─── v5.1.0: Smart port deduplication ────────────────────────────────────────

def deduplicate_ports(port_infos: list[PortInfo]) -> list[PortInfo]:
    """
    Merge PortInfo entries from multiple sources (rustscan, async, masscan, nmap).

    Resolution rules (highest confidence wins):
      - If the same port appears in multiple sources, keep the entry with the
        highest *confidence*; fill in missing service/product/version from
        lower-confidence entries if the winner has them blank.
      - The *source* field on the winner lists all contributing sources.
    """
    by_port: dict[int, PortInfo] = {}

    for pi in port_infos:
        existing = by_port.get(pi.port)
        if existing is None:
            by_port[pi.port] = pi
        else:
            # Merge sources list
            sources = set(existing.source.split(",")) | {pi.source} if pi.source else set()
            if pi.confidence >= existing.confidence:
                # pi wins — keep richer service info from existing if pi is blank
                winner = pi
                if not winner.service and existing.service:
                    winner.service = existing.service
                if not winner.product and existing.product:
                    winner.product = existing.product
                if not winner.version and existing.version:
                    winner.version = existing.version
                if existing.scripts:
                    winner.scripts.update(existing.scripts)
            else:
                winner = existing
                if not winner.service and pi.service:
                    winner.service = pi.service
                if not winner.product and pi.product:
                    winner.product = pi.product
                if not winner.version and pi.version:
                    winner.version = pi.version
                if pi.scripts:
                    winner.scripts.update(pi.scripts)
            winner.source = ",".join(sorted(s for s in sources if s))
            by_port[pi.port] = winner
