"""
ReconNinja v5.1.0 — Report Generation
Produces JSON, HTML (dark UI dashboard), Markdown, CSV, and SARIF reports.
"""
...
def generate_json_report(result: ReconResult, path: Path) -> None:
    ...
    payload = {
        ...
        # v5.1.0
        "censys_results":   result.censys_results,
        "hunter_results":   result.hunter_results,
...
<header>
  <div class="ninja">⚡ RECON NINJA v5.1.0</div>
...
def generate_markdown_report(result: ReconResult, path: Path) -> None:
    total_open = sum(len(h.open_ports) for h in result.hosts)
    lines = [
        f"# ReconNinja v5.1.0 Report — `{result.target}`", "",
        "## Summary", "",
        f"| Field | Value |",
        f"|---|---|",
...
def generate_csv_report(result: ReconResult, path: Path) -> None:
    """Export a flat CSV of all host/port/finding rows."""
    rows: list[dict] = []
    ...
    # Vuln findings
    for v in result.nuclei_findings:
        rows.append({
            "type":       "vuln",
            "target":     result.target,
            "host":       v.target,
    ...
    if not rows:
        rows.append({
            "type": "summary", "target": result.target,
            "host": "", "ip": "", "port": "", "protocol": "", "state": "",
            "service": "", "product": "", "version": "", "severity": "",
            "confidence": "", "source": "", "detail": "no findings", "cve": "",
        })

    fieldnames = [
        "type", "target", "host", "ip", "port", "protocol", "state",
        "service", "product", "version", "severity", "confidence",
        "source", "detail", "cve",
    ]
...
def generate_sarif_report(result: ReconResult, path: Path) -> None:
    """
    Export findings as SARIF 2.1.0 for GitHub Code Scanning / CI integration.
    """
    rules: list[dict] = []
    results: list[dict] = []
    rule_ids: dict[str, int] = {}

    def _get_rule(rule_id: str, name: str, severity: str) -> int:
        if rule_id not in rule_ids:
            idx = len(rules)
            rule_ids[rule_id] = idx
            rules.append({
                "id": rule_id,
                "name": name,
        ...
        return rule_ids[rule_id]

    for v in result.nuclei_findings:
        rid = v.cve if v.cve else v.tool + "/" + v.title.replace(" ", "_")[:40]
        _get_rule(rid, v.title, v.severity)
        results.append({
            "ruleId":  rid,
            "ruleIndex": rule_ids[rid],
            "level":   _SARIF_LEVEL.get(v.severity, "warning"),
            "message": {"text": v.details or v.title},
            "locations": [{
                "physicalLocation": {
            ...
            "properties": {"severity": v.severity, "cve": v.cve},
        })
...
def generate_html_from_template(
    ...
    """
    Render an HTML report using a user-supplied Jinja2 template.

    The template receives the following context variables:
        result       — ReconResult dataclass instance
        app_name     — "ReconNinja"
        version      — VERSION string
        now          — current datetime string

    Falls back to generate_html_report() if Jinja2 is not installed.
    """
    try:
        from jinja2 import Environment, FileSystemLoader, select_autoescape  # type: ignore
    except ImportError:
        from utils.logger import log
        log.warning("jinja2 not installed — falling back to default HTML report")
        generate_html_report(result, output_path)
        return

    env = Environment(
        loader=select_autoescape(["html", "jinja2"]),
        autoescape=True,
    )
    # Override loader to allow arbitrary path
    from jinja2 import FileSystemLoader as FSL
    env.loader = FSL(str(template_path.parent))
...
def build_severity_summary(result: ReconResult) -> str:
    """
    Return a one-line severity summary string, e.g.:
        CRIT:2  HIGH:5  MED:8  LOW:3  INFO:12
    """
    ...
    for v in result.nuclei_findings:
        ...
        if sev in counts:
            ...
            counts["info"] += 1
    # Also count critical/high ports
