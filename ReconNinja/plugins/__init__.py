"""
ReconNinja v5.1.0 — Plugin System
...
Plugin contract:
  ...
  - Return value: may return a list[VulnFinding] — these are appended to
    result.nuclei_findings automatically (v5.1.0+)
...
Example plugin skeleton (plugins/my_plugin.py):
    ...
    def run(target, out_folder, result, config):
        ...
        findings.append(VulnFinding(
            tool="my_plugin", severity="high",
            title="Found something", target=target,
        ))
        return findings  # v5.1.0: returned findings are auto-appended
"""
...
def run_plugins(
    ...
    for name, fn in plugins:
        ...
        try:
            returned = fn(target, out_folder, result, config)
            # v5.1.0: if plugin returns a list of VulnFinding, append them
            if isinstance(returned, list) and returned:
                from utils.models import VulnFinding
                for item in returned:
                    if isinstance(item, VulnFinding):
                        result.nuclei_findings.append(item)
                safe_print(f"[success]  ✔ {name} done (+{len([i for i in returned if isinstance(i, VulnFinding)])} finding(s))[/]")
            else:
