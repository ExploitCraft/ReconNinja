...
def parse_args() -> argparse.Namespace | None:
    ...
    parser.add_argument("--output-format", default="all",
        choices=["all","html","json","md","txt","csv","sarif"],
    ...
    parser.add_argument("--diff",          default=None,
        help="Path to a previous state.json to diff against")
    parser.add_argument("--report-template", default=None,
        help="Path to a custom Jinja2 HTML report template")

    # v5.1.0 — intelligence integrations
    parser.add_argument("--censys",        action="store_true", help="Enable Censys host intelligence")
    parser.add_argument("--censys-key",    default=None,
        help="Censys API credentials as 'api_id:api_secret'")
    parser.add_argument("--hunter",        action="store_true", help="Enable Hunter.io email enumeration")
    parser.add_argument("--hunter-key",    default=None, help="Hunter.io API key")

    # v5.1.0 — scan intelligence
    parser.add_argument("--nmap-scripts",  default=None,
        help="Comma-separated Nmap scripts or category (e.g. 'vuln' or 'http-title,ssl-cert')")
    parser.add_argument("--confidence-min", type=int, default=0,
        help="Filter findings below this confidence %% (0 = show all)")
...
def build_config_from_args(args: argparse.Namespace) -> ScanConfig | None:
    ...
    return ScanConfig(
        ...
        run_hunter      = getattr(args, "hunter", False),
        censys_key      = getattr(args, "censys_key", None) or "",
        hunter_key      = getattr(args, "hunter_key", None) or "",
        diff_file       = Path(args.diff) if getattr(args, "diff", None) else None,
        report_template = Path(args.report_template) if getattr(args, "report_template", None) else None,
        nmap_scripts    = getattr(args, "nmap_scripts", None),
        confidence_min  = getattr(args, "confidence_min", 0),
    )
