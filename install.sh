...
BANNER
echo -e "${NC}"
echo -e "${BOLD}  ReconNinja v10.2.0 — Installer${NC}"
...
AUR_HELPER=""        # BUG-FIX: initialized here — also referenced in summary outside IS_ARCH block
...
# Flask is required for the --gui / --mcp-server feature (v10: optional extra)
info "Installing Flask (GUI/MCP dependency)..."
if $PIP install flask --break-system-packages -q 2>/dev/null \
    || $PIP install flask -q 2>/dev/null; then
    ok "Flask installed"
else
    warn "Flask install failed — GUI (--gui) and MCP server will be unavailable"
fi
...
ALIAS_COMMENT="# ReconNinja v10.2.0 — https://github.com/ExploitCraft/ReconNinja"
...
if [[ -f "$HOME/.config/fish/config.fish" ]]; then
    ...
    if grep -q "alias ReconNinja" "$HOME/.config/fish/config.fish" 2>/dev/null; then
        ...
        { echo ""; echo "# ReconNinja v10.2.0"; echo "$FISH_ALIAS"; } \
...
#!/usr/bin/env bash
# ReconNinja v10.2.0 wrapper — auto-generated by install.sh
...
# BUG-FIX: fish uses 'fish_add_path' not 'export PATH' — add .local/bin to universal fish PATH
if [[ -f "$HOME/.config/fish/config.fish" ]]; then
    if ! grep -q "fish_add_path.*local/bin\|set.*fish_user_paths.*local/bin" "$HOME/.config/fish/config.fish" 2>/dev/null; then
        echo "" >> "$HOME/.config/fish/config.fish"
        echo "# ReconNinja wrapper path" >> "$HOME/.config/fish/config.fish"
        echo "fish_add_path $HOME/.local/bin" >> "$HOME/.config/fish/config.fish"
        ok "Added ~/.local/bin to fish PATH via fish_add_path"
    fi
fi
...
# ── Final tool check ──────────────────────────────────────────────────────────
step "Final tool check (v10: --check-tools is now implemented)"
echo ""
"$PYTHON" "$INSTALL_DIR/reconninja.py" --check-tools || \
    warn "Could not run --check-tools — install finished regardless"
echo ""
