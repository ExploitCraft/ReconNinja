...
def export_pdf(result: ReconResult, out_folder: Path) -> Path | None:
    ...
    try:
        ...
        _all_open = [p for h in result.hosts for p in h.open_ports]
        crit = sum(1 for v in result.nuclei_findings if v.severity == "critical")
        high = sum(1 for v in result.nuclei_findings if v.severity == "high")
        pdf.cell(0, 7, f"Subdomains: {len(result.subdomains)}  |  Open Ports: {len(_all_open)}  |  Vulnerabilities: {len(result.nuclei_findings)}  (Critical: {crit}, High: {high})", ln=True)
        pdf.ln(4)
        ...
        for vf in result.nuclei_findings[:50]:
...
<h1>🥷 ReconNinja — Security Report</h1>
<p><strong>Target:</strong> {result.target} &nbsp;|&nbsp;
   <strong>Date:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}</p>

<h2>Executive Summary</h2>
<div class="stat crit"><div class="n">{sum(1 for v in result.nuclei_findings if v.severity=="critical")}</div>Critical</div>
<div class="stat high"><div class="n">{sum(1 for v in result.nuclei_findings if v.severity=="high")}</div>High</div>
<div class="stat"><div class="n">{sum(1 for v in result.nuclei_findings if v.severity=="medium")}</div>Medium</div>
<div class="stat"><div class="n">{sum(1 for v in result.nuclei_findings if v.severity=="low")}</div>Low</div>
<div class="stat"><div class="n">{len(result.subdomains)}</div>Subdomains</div>
<div class="stat"><div class="n">{len(_all_open)}</div>Open Ports</div>
...
<h2>Open Ports</h2><p>{", ".join(f"{p.port}/{p.protocol}" for p in _all_open[:30]) or "None detected"}</p>
...
Generated by ReconNinja — For authorized security testing only.</p>
</body></html>"""
