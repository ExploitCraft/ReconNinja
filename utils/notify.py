"""
utils/notify.py — ReconNinja  (version → see info/version)
...
def _slack_payload(event: NotifyEvent) -> dict:
    ...
    return {
        "attachments": [{
            ...
            "footer": f"ReconNinja v{__version__}",
        }]
    }
...
def _discord_payload(event: NotifyEvent) -> dict:
    ...
    return {
        "embeds": [{
            ...
            "color":       colour_int,
            "footer":      {"text": f"ReconNinja v{__version__}"},
        }]
    }
...
def _post(url: str, payload: dict, timeout: int = 10) -> bool:
    ...
    req  = urllib.request.Request(
        url,
        data=data,
        headers={"Content-Type": "application/json", "User-Agent": f"ReconNinja/{__version__}"},
        method="POST",
    )
    try:
